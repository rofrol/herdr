...
```bash
O=~/.claude/skills/consult-stats/consult.py
...
- `--findings`: distinct claims/issues raised; `--accepted`: how many survived verification;
  `--unique`: accepted ones that neither Claude nor another model in the same round had. `unique` is the key signal.
...
Claude is scored too, once per round (all consult calls on the same question), with `self`:
