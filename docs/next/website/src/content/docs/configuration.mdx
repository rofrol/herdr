...
Set `tab_label = "title"` under `[ui]` to show, on tabs you have not renamed, the task title the tab's agent sets as its terminal title (Claude Code summarizes the current task there), cut to 16 columns. A tab shows its number until its agent sets a title. The default is `"number"`.
...
- `tab_jobs` — counts of the space's tabs whose status (`herdr tab status`) is running (`⧖ 1`) or failed (`!1`), when nonzero. A collapsed worktree group counts all its spaces. It shows background work such as `herdr-job` jobs while the space's agents are idle.
...
Rules work with text-valued built-ins and custom `$name` tokens in Agent rows, `rows_by_agent` overrides, and Space rows. Custom tokens match their reported value: `$load` reporting `"90"` selects red and bold above, `"60"` selects yellow, and `"90%"` keeps white. Unreported tokens still disappear. `state_icon` and the composite `git_status` and `tab_jobs` accept fixed styles only, not rules. Unknown conditions and malformed rules are rejected when loading config; regex, fuzzy matching, and scripts are not supported.
...
On macOS, `system` tries `terminal-notifier` first and falls back to `/usr/bin/osascript` when it is unavailable or fails. The fallback appears as Script Editor in Notification Center and cannot activate the hosting terminal. Install `terminal-notifier` with `brew install terminal-notifier`. For a supported, detected terminal, it can activate the terminal app when you click the notification. Agent notifications show the agent's task (its terminal title) with the workspace and tab as a subtitle, replace the previous notification from the same pane, and clicking one focuses the agent's pane on the local session. Alternatively, choose `terminal` to let a supported outer terminal own the notification.
